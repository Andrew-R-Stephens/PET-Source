...
fun RealtimePlot(
    ...
    val viewportY = state.viewportYInterval
    val viewportX = state.viewportXInterval
    ...
    Canvas(
        ...
        clipRect(
            ...
            taps.forEach { tap ->
                drawCircle(
                    color = colors.instant.copy(alpha = 1f),
                    radius = 4f,
                    center = Offset(
                        x = size.width - ((now - tap.pX) * xPlotRatio),
                        y = size.height * (1f - (tap.pY / viewportY))
                    )
                )
            }
        }
