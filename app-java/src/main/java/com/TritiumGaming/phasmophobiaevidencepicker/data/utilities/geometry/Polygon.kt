...
class Polygon : Shape, Serializable {
    ...
    fun updateBounds(x: Int, y: Int) {
        if (x < rectBounds!!.x) {
            rectBounds!!.width = rectBounds!!.width + (rectBounds!!.x - x)
            rectBounds!!.x = x
        } else {
            rectBounds!!.width = max(rectBounds!!.width.toDouble(), (x - rectBounds!!.x).toDouble())
                .toInt()
            // bounds.x = bounds.x;
        }

        if (y < rectBounds!!.y) {
            rectBounds!!.height = rectBounds!!.height + (rectBounds!!.y - y)
            rectBounds!!.y = y
        } else {
            rectBounds!!.height = max(rectBounds!!.height.toDouble(), (y - rectBounds!!.y).toDouble())
                .toInt()
            // bounds.y = bounds.y;
        }
    }
    ...
    internal class PolygonPathIterator(var poly: Polygon, var transform: AffineTransform?) :
        PathIterator {
        var index: Int = 0
