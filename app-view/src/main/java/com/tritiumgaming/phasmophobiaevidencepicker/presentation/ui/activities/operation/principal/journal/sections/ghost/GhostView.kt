...
@Deprecated("Replaced with GhostListItem", ReplaceWith("GhostListItem"), DeprecationLevel.WARNING)
class GhostView : ConstraintLayout {

    private var investigationViewModel: InvestigationViewModel? = null
    var ghostModel: GhostModel? = null
    ...
    fun build(
        ...
        ghostModel?.let { ghost ->
            ...
            nameView.text = ghost.name.let { resId -> context.getString(resId) }
            ...
            iconRowLayout?.let { layout ->
                ...
                var k = 0
                while (k < ghost.evidence.normalEvidenceList.size) {
                    val currentEvidence = ghost.evidence.normalEvidenceList[k]
                ...
                while (k < layout.childCount) {
                    layout.getChildAt(k).visibility = GONE
                    k++
                }
            }
        }
        ...
        addOnLayoutChangeListener { _: View?, _: Int, _: Int, _: Int, _: Int,
            ...
            ghostModel?.let { ghost ->
                ...
                iconRowLayout?.let { layout ->
                    ghost.evidence.normalEvidenceList.forEachIndexed { index, evidenceModel ->
    ...
    fun forceUpdateComponents() {
        ...
        ghostModel?.let { ghostData ->
            ghostData.evidence.normalEvidenceList.forEachIndexed { index, evidence ->
                val evidenceIcon =
